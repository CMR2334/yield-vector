...
- **R95 (v2026.08.23 — owner directive batch: picker-first date taps + Type control, DoC URL merge, private entity catalogs, hold-first chooser):**
  ...
  **[i] Date-input picker-primary.** Offer-add/edit + event-add/edit modal date bubbles: first tap opens custom `yv-date` DatePicker only (iOS keyboard suppressed via `inputmode="none"` + disabled state). Second tap on the active bubble disarms picker and enables iOS numeric keypad — direct-type is opt-in on subsequent taps (owner phone screenshot 2026-08-23: keypad + AutoFill covering the picker). `readonly` input + explicit tap handlers lock the interaction; tap-1 picker open + tap-2 keypad raise verified E2E on iOS 26.5 simulator (`5286b0c` commit).
  **[ii] DoC URL single-field merger.** Redundant "URL" import bubble + "DoC URL" field merged into one **"DoC URL"** label, surfaced on main modal (promoted from Advanced), import runs directly from its value (Worker fetch or pasted post body). Old bubble removed. Owner: *"having a 2nd input/url bubble seems redundant."*
  **[iii] Entity/email private catalogs + auto-defaults.** Offer detection (manual entry or DoC import) classifies as BUSINESS or PERSONAL, prefills from synced Settings defaults (business pair + personal pair). All values overridable. Catalogs stored in app state (Settings) — NEVER hard-coded source — ship empty with one-time prompt to configure. **Privacy: this repo public, owner values entered at build only.** MigrateEntityCatalogs idempotent rebuild from user's own offers on first load; settings.entityDefaults and settings.emailDefaults ship EMPTY (inert until owner sets the two Settings picker pairs). PII grep over js/ + index.html + sw.js = 0 hits re-verified at release.
  **[iv] Hold-first chooser.** Offers with both hold + card-spend paths (e.g. Brex either/or) now show radio buttons "Hold funds" first, "Card spend" second (owner preference).
  **[v] Codex Sol adversarial FAIL → 5 findings folded:** version bump (this step), hold-first render order (done), pointerType rework (unverifiable re-attempted; remained unverifiable), DoC single-field merge (done), and entity private storage (done). 4 rejected with reasons documented.
  **Pins:** 86→108 optimizer + 9 catalog, feasibility 21, parser 24, fidelity 67; beam-perf pin known flake (2200ms budget, fails ~2256–2400 under load, passes idle; budget unchanged). Battery at step 3 all green. `node --check` 19 modules + sw.js PASS.
  **Stable-2026-07-29 CUT at `fd37c2c`** by owner word (first stable tag since 07-09); simulator+desktop QA R93 all green, device pass in progress.
  **Codex Luna worker dispatch wedged** (headless approval stall) → release executed by fallback Claude worker, recorded.
  **Owner POST-RELEASE ACTION:** set the two Settings default pairs (business entity/email, personal entity/email) and confirm the catalog reconstituted from his offers on first load.
  **HANDOFF carries:** orchestrated run log + device QA evidence; pre-existing modal-boot race (opened <500ms after boot wiped by deferred render, found 2026-08-23 during device QA, backlog added).
...
- **R93 (v2026.07.29 — simulator+desktop QA release: 3 P2 layout fixes, 13 P3 polish, Codex folds):**
  ...
  **Three P2 layout bugs, fixed:** **(1)** the hero chart's frozen Y-axis column had a fixed width that clipped long labels and hid minus signs on negative balances — now content-aware, `--yv-yaxis-w` clamped 36–88px to its widest rendered label. **(2)** the desktop header (wordmark + 5 nav pills + sync chip + "Add offer") has an intrinsic 805px minimum; every viewport in the 721–804px band scrolled sideways with "Add offer" hanging off the edge — the compact/phone header now takes over at the 804/805 breakpoint instead of 720/721, so that band (**iPad mini portrait, 744pt, included — OWNER-VISIBLE change**) now renders the phone layout. **(3)** the offers grid's two-column breakpoint fired at 481px but the card's pill-chain of stat labels didn't actually fit two-up until ~560px, so 481–518px pushed the whole page into sideways scroll — `.ddm` (the pill chain) gained `min-width:0` so it can shrink, and the threshold moved to 560px (**519–559px is now single-column**, deliberate trade for zero overflow).
  **13 P3 polish items fixed:** ≥44pt touch hit-areas on coarse pointers via an invisible `::before` expansion (visual sizes unchanged); top/left/right `env(safe-area-inset-*)` respected by the header + main gutters; date inputs gained `readonly` + tap-to-open wiring so the mobile keyboard no longer auto-opens for add-offer/event/commitment modals; timeline row-label dates wrap instead of ellipsising; a recurrence end-before-start date now shows an inline warning and blocks save; duplicate date-violation hints across one form are deduped to one; the picker's "unavailable" legend only renders when the visible month actually has blocked days; stray "1 drafts" plural fixed to "1 draft"; Settings' projection-start field switched from a bare native date input to the app's standard DatePicker component (Clear resets to today); the FAB no longer overlays the last card's content at the bottom of a scrolled list; the commitments section heading no longer crowds its "+ Add commitment" button; plus 2 Codex-folded findings below.
  **Codex adversarial re-review verdict FAIL → folded in `a4c7553`** (independent second-opinion pass on the P2/P3 diff, not self-review): **(H)** the new safe-area side gutters pushed the desktop header's fit threshold from 805px to 869px, overflowing every notched-iPhone landscape width — a regression INTRODUCED by this diff, not pre-existing; side gutters restored flat, top inset kept. **(M)** 44pt checkbox hit-bands on the stacked event/commitment checkboxes overlapped their 38px-spaced neighbor — a tap near "Include in projection" could toggle the row below it; expansion removed for `.checkbox-row` (its label is already full-width, needs no hit-area assist). **(M)** an unreadable Settings projection-start date left stale garbled text on screen instead of restoring the last valid stored value — display restore fixed. Rejected with verified reasons (documented in the run log): APP_VERSION finding (belongs to step 4, out of scope then), hint-dedupe (deliberate step-2 design), landscape mobile-nav height (pre-existing, not in this diff), chart-svg 720 leftover (iPad screenshot proved the existing flex-fill renders BETTER than Codex's proposed fix), 88px Y-axis clamp ceiling (would need a ~$1e18 balance to matter).
  **Step-4 micro-fix (this release commit):** the same `(pointer: coarse)` hit-area technique, applied uniformly, gave `.btn-icon` pairs (edit/delete, edit/duplicate — 36px buttons only 4px apart, e.g. Settings' commitments/events tables and the offer/event/commitment modals) 44px-centered hit bands that overlapped their neighbor's by 4px — a tap in the sliver between edit and delete hit delete. `.btn-icon::before` now expands vertically only (still 44px tall) and stays at the button's own width horizontally, so adjacent bands meet without crossing. Verified geometrically (Playwright, real computed `::before` styles, not the naive centered-44 assumption the earlier probe used): pseudo-width == own element width for every `.btn-icon` on Overview/Planner/Offers/Settings + the offer/event modals, 0 overlaps in the sweep, and a direct repro — clicking the exact former sliver midpoint now resolves to neither button (the shared container), where it previously resolved to "delete."
  **Battery all green throughout:** optimizer 86/86, feasibility 21/21, fidelity 67/67, parser 24/24, dd-matrix PASS, p2b PASS, `node --check` 20/20 modules + `sw.js`.
  **R92 gate items resolved:** (b) hero chart right-end at a Dec-11-horizon scenario, and (e) tiny timeline-bar right-edge overrun at phone width, both verified **objectively PASS** this run. (a) neutral event-picker formatting alignment, (c) "How is this bonus met?" chooser copy order, and (f) DoC-import control placement remain **eye-call items** — evidence packs (screenshots/probes) sit in the session scratchpad for the owner to review; no code changed for these three since they need a human call, not a mechanical fix.
  **APP_VERSION → 2026.07.29** ×21 (`js/runtime-status.js`, `sw.js`, `index.html` ×19 import-map `?v=`; verified 0 strays of `2026.07.14b`).
  **Remaining owner flags (not fixed this run — noted, not actioned):** bottom-nav 44pt height option (deliberately untouched — visible layout change, owner decision per the run's own scope note); Settings events-table responsive design; action-title ellipsis; the commitment modal's two remaining native `<input type="date">` fields (not yet on the custom DatePicker); landscape cutout insets not reaching the 805–1024 desktop header band; simulator-landscape untested (rotation permission denied in this sandbox — header fold verified by probe only, not a live rotated screenshot).
  **Remaining (owner gate): device-check v2026.07.29 + the five R92 eye-call/objective items now evidenced.**

- **R92 (v2026.07.14b — requirements-derived qualification paths / Brex hold-or-spend, low-cash axis removal, timeline label fit, DoC hold-or-spend import):**
  Orchestrated run `.claude/orchestrator/runs/2026-07-13-capital-event-picker-chart-bonusmet.md`, steps 2/2b/2c/2d + fix-ups + step 3's clean verification — this is the FULL release (R91 was the interim v2026.07.14a; this closes the run's remaining scope).
  **[1] Requirements-derived qualification paths** (`js/offer-model.js`, `js/requirements-templates.js`, `js/projection-optimizer.js`, `js/optimizer-engine.js`, `js/reminders.js`, `js/render-main-views.js`, `js/modals-forms.js`; design addendum `docs/assessments/2026-07-13-requirements-driven-paths.md`): `pathState()` now derives available paths from an offer's LIVE requirement rows via a family map — `dd` (direct_deposit_amt/count), `debit` (spend/debit_txns/transactions, display label "Card spend"), `hold` (deposit/maintain_balance), plus path-neutral rows (activate_debit/estatements/online_banking/promo_code/custom) that always count. `plannedPath` gains `'hold'`, valid only for `new-funds-held` under `requirementLogic: 'any'` — models Brex's real hold-OR-card-spend terms. Every hold/DD consumer is gated on the derived `holdActive`/`ddActive`: a debit-path Brex ties up ZERO capital (projection, optimizer, safe-to-close, reminders, checklist counts, requirement deadlines all agree); a hold-path Brex still models the block. `held-and-dd` keeps its hold UNCONDITIONAL (footprint assertion, not part of the either/or) — its chooser stays dd-vs-debit only. `'all'`-logic offers are byte-identical to pre-change (no schema/migration bump — loose fields). Chooser UI (`reqMetPaths`/`reqMetSectionField`/`syncReqMetSection`) now shows when ≥2 path families are present among the requirement rows, with truthful per-variant copy (fixed the stale "e.g. Brex" DD example; held variant reads "Either way (hold funds or card spend)").
  **4 adversarial-review findings folded before verification:** (H1) `offer-model.js` — `'all'`-logic `holdActive` was gated by a `HELD_LUMP_TYPES` allow-list, which silently dropped the held-lump modeling for any legacy/undefined `offerType` (the pre-existing seed/legacy case reminders.js explicitly guards); restored to the literal `offerType !== 'direct-deposit'`. (H2) `optimizer-engine.js` + `render-main-views.js` — the deposit-deadline constraint, horizon push, and hero-chart marker still enforced/showed a funding deadline on a DORMANT hold path (debit-path Brex); now gated on `holdActive`. (M1) `offer-model.js` — `bonusWindowAnchor` now sweeps `requirementActive()` so a completed but dormant-path row can't anchor the expected-bonus window/safe-to-close months late. (M2) `offer-model.js` — `allRequirementsDone` now stays false while `'any'`-logic + `plannedPath: null` (needsPath), instead of wrongly reporting met-waiting off neutral rows alone.
  **[2] Low cash removed as an optimizer survival axis** (`js/optimizer-engine.js`, `js/render-main-views.js`; owner-directed — trusts the buffer hard gate, "okay removing it and trusting it won't go below my already-present cash buffer"): dropped from `altWeaklyDominates`/`altStrictlyBetterSomewhere`/`altEdgeVsHeadline` gains and `formatAltEdge`'s "+$X low cash" label — a 1-offer cushion-only alternative no longer survives Pareto filtering on low cash alone. Buffer stays the hard feasibility gate; `altMetricLowCash` + the card's low-cash line + `capitalCurveSummary` stay as an informational metric, untouched.
  **[3] Timeline bar minimum width for label fit** (`js/render-main-views.js`, `index.html`; owner-directed): bars with on-bar text (currency / "DD") get content-driven `min-width: max-content` via new `.tl-bar.has-label`, applied under the same width gate that decides the label renders — start edge anchored, textless slivers keep the 28px floor, normal bars byte-identical.
  **[4] DoC importer hold-or-spend either/or** (`js/doc-import-templates.js`, `js/doc-parser.js`): the DoC parser gained a hold-term detection bridge (hold new funds / maintain balance) parallel to the existing DD bridge, sharing the SAME sentence-scoped fee-waiver guard (fixed from a whole-post-scoped false negative that suppressed detection on almost any real post with an ordinary monthly-fee row). The importer's force-DD decision (`_docEitherOrForceDdFamily`) now derives from the user's CHECKED preview row + its parsed value, not the raw unchecked parse — a hold-or-spend import surfaces a real Hold-funds/Card-spend chooser instead of an inapplicable forced DD path.
  **Battery: optimizer 86/86 (beam 2059ms), feasibility 21/21, parser 24/24, fidelity 67/67, dd-matrix + p2b PASS; node --check clean on all modules + sw.js.** **Step 3 rendered-DOM E2E** (Playwright + Chromium, seeded fresh local origin, owner data untouched): **63/63** assertions across all six change groups, zero console errors, zero horizontal overflow (desktop + 380px) — confirmed debit-path Brex zero-capital, hold-path block modeling, "Dec Dec 11" gone at a Nov-DST horizon, neutral picker clean, invalid recurrence end rejected, hold-or-spend import (with fee row) detects/stays held/chooser shows both paths unpicked, DD-unchecked paste doesn't force DD, no cushion-only alternative survives, "$1.2M" bar grows 28→50px unclipped with start anchored.
  **APP_VERSION → 2026.07.14b** ×3 (`js/runtime-status.js`, `sw.js`, `index.html` ×19 import-map `?v=`; 0 strays of 14a).
  **Remaining (owner gate): device-check v2026.07.14b:**
  (a) neutral event-picker formatting alignment — eye-call vs the optimization-colored pickers elsewhere; (b) hero chart right-end reads clean at a Dec-11-horizon scenario; (c) "How is this bonus met?" chooser copy order is "Card spend or Hold funds" — one-line cosmetic flip available if hold-first reads better; (e) tiny timeline-bar right-edge overrun is acceptable per spec — confirm $-label fit at phone width; (f) DoC import control lives inside collapsed Advanced fields (pre-existing placement, not new) — flag if you want it more prominent.

- **R91 (v2026.07.14a — owner batch: neutral event date-picker + "Dec Dec 11" DST label fix + dynamic "How is this bonus met?" + posting-window hint):**
  Orchestrated run `.claude/orchestrator/runs/2026-07-13-capital-event-picker-chart-bonusmet.md` — this is its INTERIM release (step 1 + review fixes shipped early because the owner was waiting on visible fixes); the big Step 2 (requirements-derived qualification paths, design pinned in the checkpoint) is still pending with its own verification + release to follow.
  **[1] Neutral capital-event picker** (`js/dd-widgets.js`, `js/modals-forms.js`): the Add/Edit-event Date + recurrence-Ends fields switched from native `<input type="date">` to the custom `yv-date` DatePicker with a NEW `mode: 'neutral'` — identical popover formatting, NO optimization colorization (no good/amber/holiday classes, no optimality legend; only selection + blocked states). Save path parses M-D-YYYY display → ISO via `parseDateInput` (`readEventForm`: `date`, `recurrence.endDate`).
  **[2] "Dec Dec 11" hero-chart bug** (`js/render-main-views.js`): the horizon-end label suppression reconstructed dates via `start + i*86400000` — DST drift landed the last month-tick in the PREVIOUS month, defeating the same-month guard so both "Dec" and "Dec 11" rendered. x-ticks now carry their true first-of-month `Date`; horizon date comes from the last projection day.
  **[3] Dynamic "How is this bonus met?"** (`js/modals-forms.js`): the EITHER/OR chooser (`reqMetPaths`/`reqMetSectionField`/`syncReqMetSection`) renders ONLY when the offer has BOTH a DD path (DD-family type) and a debit path, and rebuilds live on offerType/debitRequired changes preserving an in-progress choice. A held-only offer (owner's Brex complaint) no longer sees an inapplicable Direct-deposit chooser; with the radios absent, `readOfferForm` falls back to `requirementLogic 'all'`/`plannedPath null`. NOTE: Brex is actually hold-OR-spend per owner — full modeling lands in Step 2.
  **[4] Posting-window hint** (`js/modals-forms.js`, tier-worker): the advanced "Bonus posting window" field-hint enriched — anchor (latest requirement `done_date`, estimated from today until recorded), interpolated `${DEFAULT_BONUS_POST_MIN_DAYS}`–`${DEFAULT_BONUS_POST_MAX_DAYS}` defaults, drives expected-bonus window + reminders + safe-to-close END, overridden by `bonus_received_date`.
  **Codex read-only review (2 findings, both FIXED pre-release):** (H) DoC-import `requirementLogic 'any'` apply grabbed `#reqlogic-any` BEFORE establishing the paths that make the dynamic section render — reordered so offerType/debit flip first (`js/doc-import-templates.js`); (M) a non-empty unparsable recurrence end date silently saved as "no end" → validated in `saveEventFromForm` (`js/events-actions-data.js`). Codex workspace-write dispatches HUNG twice (background-unanswerable approval; read-only + smoke tests fine) → step 1 fell back to the Claude tier-worker per ladder; use read-only Codex + Claude workers until resolved.
  **APP_VERSION → 2026.07.14a** ×3 (js/runtime-status.js, sw.js, index.html ×19 import-map `?v=`; 0 strays of 13b). node --check clean on all touched modules. Full behavioral pass (rendered-DOM incl. mobile) still OWED — runs with Step 2's verification.
  Also: `docs/BACKLOG.md` gained live-bank-data integrations (SimpleFIN research: docs/assessments/2026-07-13-bank-data-aggregation-research.md), the requirements-driven-paths phasing, an owner-action-pending section, and typography PARKED (owner wants real Calibre or nothing — never re-pitch lookalikes).

- **R90 (v2026.07.13b — TWO owner-directed features: date-picker impossible-date graying + stale-prospect dates treated as today):**
  **FEATURE 1 — DATE-PICKER IMPOSSIBLE-DATE GRAYING** (owner: *"gray out dates … no longer possible … planned funding when sign up precedes it … historical sign-up stays allowed, but subsequent actions must be after"*).
  A per-field CONSTRAINT map (ISO floor/ceiling) now drives the custom `DatePicker` (`js/dd-widgets.js`): days outside `[floor, ceiling]` render **muted + `disabled` (no `data-day`)** — visibly and functionally unavailable — and a new **"unavailable"** legend entry shows only when the field is constrained. `applyDateConstraints(form)` (`js/modals-forms.js`) derives the bounds from the LIVE form values (not saved state, so mid-edit changes apply) and writes them onto each input's `data-floor`/`data-ceiling`/`data-ceiling-label`: **planned funding** → floor = entered sign-up, ceiling = deposit deadline (sign-up + daysAfterSignupAllowedBeforeDeposit) when derivable; **DD initiation dates** → floor = sign-up (no ceiling); **planned sign-up** → NO floor (historical entry fully allowed), ceiling = offerExpirationDate when entered. Re-runs on field init + every relevant input/change + after each DD-row rebuild (add/remove/generate). A gentle **typed/pasted-violation hint** (`updateDateViolationHint`, reuses the `.field-hint` idiom in a `.yv-date-warn` danger tone) appears when a typed date breaks its bound (the picker grays, but typing mustn't be a silent loophole) and clears when valid. CSS: `.yv-dp-day.blocked` (+ struck-through numeral), `.yv-dp-legend .s.blocked`, `.field-hint.yv-date-warn` in `index.html`.
  **FEATURE 2 — STALE PROSPECT DATES TREATED AS TODAY** (owner: *"move forward the sign up date to current day instead of allowing it to become historical … if it collides with expiration it's no longer adjusted past that"*; planner-confirmed **effective-date, no silent storage mutation** — a boot-time auto-write would fire sync pushes from every device and race the Gist). THE single pure helper `effectiveOfferForToday(offer, todayISO)` + `mapEffectiveOffers` (`js/offer-model.js`): for PRE-ACCOUNT offers only (`HYPOTHETICAL_OFFER_STATUSES` = prospect/selected — the exact partition the projection buckets as hypothetical; confirmed/open/historical NEVER shift) whose signup < today, the WHOLE date group (signup + optionalPlannedFundingDate + directDeposits[].plannedDate) shifts forward by ONE calendar delta so signup lands on today — the P1-3 group-shift semantics (uniform `addDays(delta)`; model functions apply their own business-day normalization downstream; DD ids preserved). **EXPIRY COLLISION**: today strictly past offerExpirationDate → NOT advanced (`windowPassed`, surfaced as needs-attention). Consumers cannot disagree because each maps through the ONE helper with the SAME reference: **generateProjection + effectiveHorizonDays** (`js/projection-optimizer.js`) map internally keyed on `projectionStartDate` (the projection's auto-rolled "now"; `mapEffectiveOffers` returns the SAME array ref when nothing shifts, so the whole existing pin battery stays byte-identical); **timeline bars** (`renderTimeline`) and **offer card** (`renderOfferCard`, effective date display) in `js/render-main-views.js`; **optimizer input** (`buildOptimizerInput`) in `js/events-actions-data.js`. **CARD UX**: a stale offer shows a `chip-warn` *"date passed — treated as today"* + a one-tap **"Move to today"** (`moveOfferToToday`) that PERSISTS the shifted group via modal-save semantics (last_edited, reconcileClosedDate, syncRequirementsWithLegacy, DD-id preservation) — user-initiated only, and deliberately does NOT force includeInScenario. `windowPassed` shows a `chip-danger` *"sign-up window passed"*; the optimizer already surfaces it as a **no-valid-date-window** review row (candidateWindow `end(expiry) < start(today)` → empty grid), tap-through works — no engine change. **PINS: feasibility 12→17** (Cstale-A effective-today projection: stale prospect ties up from today, curve dips to 20k not free 50k; Cstale-B group-shift coherence: signup→today, funding+DD offsets preserved, DD ids kept; Cstale-C expiry collision needs-attention + future/edge shift; Cstale-D confirmed/open never shifted, prospect & selected both shift; Cstale-E determinism + idempotence). **All prior 12 feasibility + 83 optimizer + 20 parser + 67 fidelity + p2b + dd-matrix stayed GREEN untouched** — every existing pin's prospect sits on/after its projectionStartDate, so the `signup < projectionStartDate` guard never fires (no-op, same array ref). **Battery: feasibility 17/17, optimizer 83/83, parser 20/20, fidelity 67/67, p2b PASS, dd-matrix PASS; node --check all clean.** **APP_VERSION → 2026.07.13b** ×3 (js/runtime-status.js, sw.js, index.html ×19 import-map `?v=`; sw precache derives yv-precache-2026.07.13b; **0 strays** of 13a). **Preview E2E** (Browser pane, port 8765, seeded a fresh local origin — owner's real data on the deployed site + Gist untouched): stale prospect card read *"Fund date Jul 13 · Capital back Jul 29"* (shifted from Jun 3) + the chip + Move-to-today; **Move-to-today persisted** signup/funding → 2026-07-13 with last_edited, chip + button gone; timeline bar re-anchored **Jul 13 → Jul 29**; **Home: TIED UP TODAY $30,000 / LOWEST $55,000 Jul 23** (stale no longer free capital); expired-window card showed *"sign-up window passed"* + raw Jun dates + NO Move button; future prospect untouched; funding picker grayed Jul 1–19 (before floor) + Jul 31 (after deposit deadline), Jul 20–30 available; sign-up picker allowed June/July fully (historical, no floor) + grayed Aug 16+ (past expiration); typed-violation hint fired for before-floor + after-ceiling and cleared in-range; DD-initiation floor = sign-up; **380px zero horizontal overflow; zero console errors**. **REVIEW-AFTER — Codex adversarial UNAVAILABLE** (installed codex-cli 0.135.0 rejects the profile's default `gpt-5.6-sol` and a temp `CODEX_HOME` pinned to gpt-5.5 still resolved gpt-5.6-sol; no `~/.codex-backup`) → fell to the ladder's **independent adversarial Claude** (never self-review; RUNG 3). It ran both pin batteries green and confirmed all four named consumers consistent, finding **ONE P2 FOLDED pre-push**: `renderHeroChart` drew its curve from the now-shifted `generateProjection` but computed its offer MARKERS (fund/withdraw/DD/deadline dots) from RAW dates — a curve-vs-marker disagreement on the flagship screen. Fixed identically to the timeline bars: the marker loop iterates `mapEffectiveOffers(App.state.offers, projectionStartDate)`. Verified: with a stale prospect the hero now shows its fund marker AT today (cx 8.0 == the today dot) + a withdraw marker, 4 dots total (both offers), vs the pre-fold 2 (stale dropped as i<0). Also folded a P3: `mapEffectiveOffers` now materializes its output array LAZILY (zero allocation in the no-shift case). Owner-owned paths (`.github/`, `AGENTS.md`, `CLAUDE.md`, `.claude/`) untouched — explicit-path `git add` only. **Remaining (owner gate): device-check v2026.07.13b.**
